...
bool ProjectionTdFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    // 通过传感器时间延时td，更新特征点。当使用全局曝光的相机TR为0
    // td_i td_j 分别表示第i帧和第j帧的传感器时延
    // 这里td是最新估计的传感时间延时，td - td_i/td_j 表示的是两个时间时延的差值。在实际情况中，是固定IMU的时间戳，而移动相机的时间戳的，因为移动IMU的时间戳需要重新预积分
