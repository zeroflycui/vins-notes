...
class IMUFactor : public ceres::SizedCostFunction<15, 7, 9, 7, 9>
{
  public:
    ...
    * parameters 是一个4*7的参数块
    * residuals 残差的计算结果，是一个一维数组，残差就是该观测量和约束的状态量通过某种关系形成残差
    * jacobians 残差对参数块的雅克比矩阵也是一个二维数组，对任意一个参数块的雅克比矩阵都是一个一维数组
    */
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {

        // 便于后面的计算，把参数块都转换为eigen
        // imu预积分约束的参数是相邻两帧的位姿，速度和零偏
        // 前一帧
...
//delta_q = Qi.inverse() * Qj;
        ...
        residual = pre_integration->evaluate(Pi, Qi, Vi, Bai, Bgi,
                                            Pj, Qj, Vj, Baj, Bgj);
        // e^TPe ===》 e^Tll^Te P是信息矩阵（协方差的倒数），e是残差
        // 因为ceres没有g2o设置信息矩阵(P)的接口，因此置信度直接乘在残差上，这里通过LLT分解，相当于将信息矩阵开根号 
        // pre_integration->covariance协方差矩阵
        // sqrt_info 是l^T
