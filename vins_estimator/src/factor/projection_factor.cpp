...
bool ProjectionFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    // 将第i帧相机下的测量点（恢复深度）变换到世界坐标系下然后再转换到第j帧相机帧中，完成重投影点的计算
    Eigen::Vector3d pts_camera_i = pts_i / inv_dep_i; // 第i帧相机坐标系下坐标（恢复深度后的坐标）
    Eigen::Vector3d pts_imu_i = qic * pts_camera_i + tic; // 第i帧imu坐标系下坐标
    Eigen::Vector3d pts_w = Qi * pts_imu_i + Pi; // 世界坐标系
    // 转到第j帧imu坐标系。其中pts_w - Pj，pts_w表示第i帧imu对应的世界坐标，Pj表示第j帧imu到世界坐标系的平移。两者相减就得到第j帧imu对应的世界坐标系
    Eigen::Vector3d pts_imu_j = Qj.inverse() * (pts_w - Pj); // 得到第j帧imu下的点
    ...
    residual = (pts_camera_j / dep_j).head<2>() - pts_j.head<2>(); // 重投影误差（重投影特征 - 光流估计特征）
#endif
    // 重投影的信息矩阵是不变的
    residual = sqrt_info * residual;  // 误差乘上信息矩阵（1.5个像素的虚拟焦距）
    ...
    if (jacobians)
    {
...
#else
        reduce << 1. / dep_j, 0, -pts_camera_j(0) / (dep_j * dep_j),   // 重投影误差对估计的j帧下相机坐标系下坐标变量求导
        ...
        if (jacobians[0])
        {
            ...
            jaco_i.leftCols<3>() = ric.transpose() * Rj.transpose();  // 重投影对第i帧平移的求导
            jaco_i.rightCols<3>() = ric.transpose() * Rj.transpose() * Ri * -Utility::skewSymmetric(pts_imu_i);  // 重投影对第i帧旋转的求导
