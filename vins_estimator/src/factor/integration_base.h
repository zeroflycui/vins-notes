...
{
  public:
    IntegrationBase() = delete; //不使用默认构造函数
...

    {
        ...
        noise.block<3, 3>(0, 0) =  (ACC_N * ACC_N) * Eigen::Matrix3d::Identity(); // na(k)
        noise.block<3, 3>(3, 3) =  (GYR_N * GYR_N) * Eigen::Matrix3d::Identity(); // nb(k)
        noise.block<3, 3>(6, 6) =  (ACC_N * ACC_N) * Eigen::Matrix3d::Identity(); // na(k+1）
        noise.block<3, 3>(9, 9) =  (GYR_N * GYR_N) * Eigen::Matrix3d::Identity(); // nb(k+1)
        noise.block<3, 3>(12, 12) =  (ACC_W * ACC_W) * Eigen::Matrix3d::Identity(); // nba
        noise.block<3, 3>(15, 15) =  (GYR_W * GYR_W) * Eigen::Matrix3d::Identity(); // nbw
    }
    /**
     * @brief 对当前imu帧和之前帧imu帧间进行积分，同时将传入变量保存在预积分成员变量dt_buf acc_buf gyr_buf 中
     * @in dt 当前帧imu与前一帧imu的时间差
     * @in acc gyr 表示当前帧imu的测量加速度和角速度
     */
    ...
    // 中值积分
    //  传入的imu值是相邻两帧的测量值以及相邻两帧pvq的变化量
    // 得到第i+1时刻（imu）到第k帧（img）的p、v、q。 ba、bg实际上并没有改变
    /*
    @in:
        _dt, _acc_0, _gyr_0, _acc_1, _gyr_1, delta_p, delta_q, delta_v(上一次的中值定理得到的状态),
        linearized_ba，linearized_bg （新的加速度和陀螺仪零偏）
         update_jacobiar
    @out:
    ...
    */
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        // 中值积分更新状态量
        Vector3d un_acc_0 = delta_q * (_acc_0 - linearized_ba); // 对齐到第k帧
        Vector3d un_gyr = 0.5 * (_gyr_0 + _gyr_1) - linearized_bg;  // 由于陀螺仪是帧间旋转，不需要进行坐标系的转换。只需通过中值表示当前时刻的值
        result_delta_q = delta_q * Quaterniond(1,un_gyr(0) * _dt / 2, un_gyr(1) * _dt / 2, un_gyr(2) * _dt / 2); // 当旋转角度接近于零时，旋转的四元数接近（1，n*角/2）
        Vector3d un_acc_1 = result_delta_q * (_acc_1 - linearized_ba); 
        Vector3d un_acc = 0.5 * (un_acc_0 + un_acc_1); // 通过均值求得imu坐标系下第i+1时刻的加速度值
        result_delta_p = delta_p + delta_v * _dt + 0.5 * un_acc * _dt * _dt; // 在imu坐标系中从第i时刻到第i+1时刻下位置的变化量
        result_delta_v = delta_v + un_acc * _dt;    // 从第i时刻到第i+1时刻下速度的变化量
         // 认为在两帧IMU间的零偏没有变化
        ...
        if(update_jacobian)
        {
            Vector3d w_x = 0.5 * (_gyr_0 + _gyr_1) - linearized_bg; // 中值角加速度
            ...
            R_w_x<<0, -w_x(2), w_x(1),
                w_x(2), 0, -w_x(0),
                -w_x(1), w_x(0), 0; // 角加速度的反对称矩阵
            R_a_0_x<<0, -a_0_x(2), a_0_x(1),
                a_0_x(2), 0, -a_0_x(0),
                -a_0_x(1), a_0_x(0), 0; // 上一时刻的加速度的反对称矩阵
            R_a_1_x<<0, -a_1_x(2), a_1_x(1),
                a_1_x(2), 0, -a_1_x(0),
                -a_1_x(1), a_1_x(0), 0; // 当前时刻加速度的反对称矩阵
            ...
            //step_jacobian = F;
            //step_V = V;
            jacobian = F * jacobian; // 更新雅克比矩阵 Jk+1 = F*Jk
            // TODO: 求出的协方差后来具体用到什么地方了？？ 答：用在计算imu残差，通过协方差矩阵构造信息矩阵作为imu残差的权重
            covariance = F * covariance * F.transpose() + V * noise * V.transpose(); // 更新协方差矩阵
        }

    }

    // 通过预积分得到当前imu帧到第K帧KF的p、v、q
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
        ...
        // 中值积分，通过i时刻的IMU数据得到i+1时刻到第k帧(img)时刻的变化量(result_delta_p\result_delta_q\result_delta_v)
        // acc_0, gyr_0 表示上一个imu时刻的加速度和角速度
        // 传入的是i+1时刻的线性加速度和角速度（测量值）以及上一帧imu的到起始帧的变化量delta_p delta_q delta_v,加速度偏差和角速度偏差ba bg
        ...
        //                    linearized_ba, linearized_bg);
        // 更新预积分类中的成员变量——IMU数据变化量（Tk,i)，k时刻是imu坐标系下对应第K帧图像帧的时间，i表示当前imu帧
        // 表示中值定理之后的状态变量
        ...
        delta_q.normalize();
        sum_dt += dt; // 记录imu时间和，在创建imu预积分对象时初始化为零
        // 更新上一帧imu的线性加速度和角速度
        acc_0 = acc_1;
        gyr_0 = gyr_1;  
...
// 计算和给定相邻帧状态量的残差
                                          ...
                                          const Eigen::Vector3d &Pj, const Eigen::Quaterniond &Qj, const Eigen::Vector3d &Vj, const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
    {
        ...
        // delta_q、delta_p、delta_v是预积分量（通过零偏来更新预积分，避免每次都重新计算预积分）
    ...
    double dt;
    Eigen::Vector3d acc_0, gyr_0; // 上一帧IMU的加速度和角速度
    Eigen::Vector3d acc_1, gyr_1; // 当前imu帧的加速度和角速度
    ...
    double sum_dt;
    // imu坐标系下 第i帧到第k帧图像帧对应的时刻的p、q、v (Tk,i)，初始值在预积分构造函数中完成初始化p,v变为零，q变为单位向量
