...
void pubKeyframe(const Estimator &estimator)
{
    ...
    {
        int i = WINDOW_SIZE - 2; // 保证当前是关键帧的索引
        //Vector3d P = estimator.Ps[i] + estimator.Rs[i] * estimator.tic[0];
        ...
        pub_keyframe_pose.publish(odometry); // 发布上述状态量值
        ...
        for (auto &it_per_id : estimator.f_manager.feature)
        {
            ...
            // 保证观察到当前特征点的起始帧小于WINDOW-2 ,同时观察到特征点的最后一帧要大于WINDOW - 2，以及需要进行求解
            if(it_per_id.start_frame < WINDOW_SIZE - 2 && it_per_id.start_frame + frame_size - 1 >= WINDOW_SIZE - 2 && it_per_id.solve_flag == 1)
            {

                int imu_i = it_per_id.start_frame;
                Vector3d pts_i = it_per_id.feature_per_frame[0].point * it_per_id.estimated_depth; // 恢复特征点的深度（相机坐标系下）
                Vector3d w_pts_i = estimator.Rs[imu_i] * (estimator.ric[0] * pts_i + estimator.tic[0]) 
                                      + estimator.Ps[imu_i]; // 先由相机坐标系转到惯性坐标系
                geometry_msgs::Point32 p;
                p.x = w_pts_i(0);
                p.y = w_pts_i(1);
                p.z = w_pts_i(2);
...
void pubRelocalization(const Estimator &estimator)
{
    ...
    odometry.twist.twist.linear.y = estimator.relo_frame_index; // 回环帧在滑动窗口中的idx，用于确定回环帧针对哪一帧的相对位姿
