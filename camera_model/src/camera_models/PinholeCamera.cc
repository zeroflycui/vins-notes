               ...
               k1, k2, p1, p2, fx, fy, cx, cy)
{
    ...
    // 用于求像素坐标到归一化坐标转化
    m_inv_K11 = 1.0 / mParameters.fx(); // 1/fx
    m_inv_K13 = -mParameters.cx() / mParameters.fx();  // -cx/fx
    m_inv_K22 = 1.0 / mParameters.fy(); // 1/fy
    m_inv_K23 = -mParameters.cy() / mParameters.fy(); // -cy/fy
}
...
PinholeCamera::liftProjective(const Eigen::Vector2d& p, Eigen::Vector3d& P) const
{
    ...
    else
    {
        ...
        {
            // 这种去畸变相比opcv中的去畸变的优势，在耗时和精度方面进行比较
            // Recursive distortion model
            int n = 8; // 迭代次数
            
            // d_u包含畸变模型中的xd-x和yd-y
            // 代表畸变距离变量，用于迭代，直到迭代点在误差允许范围内接近目标点，即可近似求得目标坐标
            Eigen::Vector2d d_u; // d_u属于以当前归一化平面点(mx_d,my_d)畸变后的点
            // mx_d + d_u = 畸变后
            distortion(Eigen::Vector2d(mx_d, my_d), d_u);
            // Approximate value
            // 第一次得到靠近目标点的一个坐标值
            // 减d_u()的目的是为了得到临时点的坐标，因为临时点在靠近目标点的一侧。
            mx_u = mx_d - d_u(0);
            my_u = my_d - d_u(1);
...
PinholeCamera::distortion(const Eigen::Vector2d& p_u, Eigen::Vector2d& d_u) const
{
    ...
    /*
    * 畸变矫正后的坐标
    * xdistor = x(1 + k1*r^2 + k2*r^4) + 2p1xy + p2(r^2 + 2x^2)
    * ydistor = y(1 + k1*r^2 + k2*r^4) + p1(r^2 + 2y^2) + 2p2xy
    * pdistor = p + d_u ==> d_u = pdistor - p;
    */
    // 得到p_u畸变后的点d_u
                          ...
                          Eigen::Matrix2d& J) const
{
    ...
    // 相机畸变模型xd和yd分别减去x，y的结果，即 xd - x; yd - y
    d_u << p_u(0) * rad_dist_u + 2.0 * p1 * mxy_u + p2 * (rho2_u + 2.0 * mx2_u),
           p_u(1) * rad_dist_u + 2.0 * p2 * mxy_u + p1 * (rho2_u + 2.0 * my2_u);
...
PinholeCamera::setParameters(const PinholeCamera::Parameters& parameters)
{
    ...
    if ((mParameters.k1() == 0.0) &&
        ...
        (mParameters.p2() == 0.0))
    {
        m_noDistortion = true; // 如果没有畸变参数，则认为该图像没有畸变
    }
    else
    {
        m_noDistortion = false;
