...
void image_callback(const sensor_msgs::ImageConstPtr &image_msg)
{
    ...
    if(!LOOP_CLOSURE) // 不检测回环，原图也没有意义，返回退出
        return;
    m_buf.lock();
    image_buf.push(image_msg); // 将原图添加到image_buf中
    m_buf.unlock();
    //printf(" image time %f \n", image_msg->header.stamp.toSec());

    // detect unstable camera stream
    if (last_image_time == -1) // 如果上一帧图像的时间戳不存在
        last_image_time = image_msg->header.stamp.toSec(); // 更新上一帧图像的时间戳
    // 如果当前图像的时间戳与上一帧图像时间戳相差过大（有较大延时）或者当前图像的时间戳小于上一帧的时间戳（时间戳错乱），这时需要重新建立一个地图（sequence）
...
void imu_forward_callback(const nav_msgs::Odometry::ConstPtr &forward_msg)
{
    if (VISUALIZE_IMU_FORWARD)
    {
        ...
        vio_t = posegraph.w_r_vio * vio_t + posegraph.w_t_vio; // 世界坐标系下的平移
        vio_q = posegraph.w_r_vio *  vio_q; // 世界坐标系下的旋转
...
void process()
{
    ...
    while (true)
    {
        ...
        m_buf.lock();
        // 进行时间戳对齐，取出第一次对齐的原图，KF位姿以及KF对应地图点
        if(!image_buf.empty() && !point_buf.empty() && !pose_buf.empty())
        {
            // 时间戳的示意图
            // ************ image_buf
            //   ******     pose_buf\point_buf
            // 原图时间戳如果晚于KF位姿和KF对应地图点的时间戳则将其去除，保证原图时间戳对齐
            ...
            {
                // 只取出位姿队列中最老的位姿，其余位姿均丢弃
                pose_msg = pose_buf.front();
                pose_buf.pop();
                while (!pose_buf.empty()) // 清空KF位姿队列
                    pose_buf.pop();
                while (image_buf.front()->header.stamp.toSec() < pose_msg->header.stamp.toSec()) // 将原图时间戳与KF位姿时间戳对齐
                    image_buf.pop();
                image_msg = image_buf.front(); // 得到与位姿对齐的图像信息
                image_buf.pop();

                while (point_buf.front()->header.stamp.toSec() < pose_msg->header.stamp.toSec()) // 将KF地图点的时间戳与KF位姿时间戳对齐
                    point_buf.pop();
                point_msg = point_buf.front(); // 得到与位姿对齐的地图点信息
                point_buf.pop();
               
            }
        }
        m_buf.unlock();
         // 经过以上操作，原图和KF位姿以及KF地图点时间戳实现对齐。
        if (pose_msg != NULL) // 如果KF位姿不为空
        {
            //printf(" pose time %f \n", pose_msg->header.stamp.toSec());
            //printf(" point time %f \n", point_msg->header.stamp.toSec());
            //printf(" image time %f \n", image_msg->header.stamp.toSec());
            // skip fisrt few
            if (skip_first_cnt < SKIP_FIRST_CNT) // 跳过前10帧，不做处理（因为前十帧的累计漂移很小）
            {
                skip_first_cnt++;
                continue;
            }
            ...
            cv_bridge::CvImageConstPtr ptr; // 将ros -》opencv格式
            ...
            if((T - last_t).norm() > SKIP_DIS) // 要求KF相隔必要的平移距离
            {
                ...
                m_process.lock();
                start_flag = 1;
                posegraph.addKeyFrame(keyframe, 1); // 1表示当前有检测到回环
                m_process.unlock();
                frame_index++;
                last_t = T; // 更新当前关键帧的平移，作为下一次关键帧间距离的条件
            }
        }
...
void command()
{
    if (!LOOP_CLOSURE) // 只有进行回环检测才有意义
        return;

    // 不断的轮询键盘是否输入“s”命令，用来保存地图
    while(1)
    {
        char c = getchar();
        if (c == 's') // 当检测到有“s”键入，则开始保存地图
        {
            m_process.lock();
...
int main(int argc, char **argv)
{
    ...
    n.getParam("skip_cnt", SKIP_CNT); // 跳过前两帧
    n.getParam("skip_dis", SKIP_DIS); // 两帧距离门限
    std::string config_file;
    n.getParam("config_file", config_file);
    cv::FileStorage fsSettings(config_file, cv::FileStorage::READ); // 读配置文件
    ...
    cameraposevisual.setScale(camera_visual_size); // 设置相机可视化模型的大小
    ...
    if (LOOP_CLOSURE)
    {
        ...
        string vocabulary_file = pkg_path + "/../support_files/brief_k10L6.bin"; // 训练好的词袋二进制文件路径
        cout << "vocabulary_file" << vocabulary_file << endl;
        posegraph.loadVocabulary(vocabulary_file); // 加载二进制词袋文件

        BRIEF_PATTERN_FILE = pkg_path + "/../support_files/brief_pattern.yml"; // 计算描述子pattern的文件
        cout << "BRIEF_PATTERN_FILE" << BRIEF_PATTERN_FILE << endl;
        // 生成一个针孔相机模型
        // c_str()将string转换为一个c类型的const字符串
    ...
    ros::Subscriber sub_image = n.subscribe(IMAGE_TOPIC, 2000, image_callback); // 原图的回调
    ...
    pub_match_points = n.advertise<sensor_msgs::PointCloud>("match_points", 100); // 发送回环匹配点

    std::thread measurement_process; // 主线程
    std::thread keyboard_command_process; // 接收键盘值回调的线程
